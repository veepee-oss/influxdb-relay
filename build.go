...
var (
	...
	race                  bool   = false
	workingDir            string
	serverBinaryName      string = "influxdb-srelay"
)
...
func makeLatestDistCopies() {
	...
	runError("cp", "dist/influxdb-srelay_"+version+"_amd64.deb", "dist/influxdb-srelay_latest_amd64.deb")
	runError("cp", "dist/influxdb-srelay-"+linuxPackageVersion+rpmIteration+".x86_64.rpm", "dist/influxdb-srelay-latest-1.x86_64.rpm")
	runError("cp", "dist/influxdb-srelay-"+version+".linux-x64.tar.gz", "dist/influxdb-srelay-latest.linux-x64.tar.gz")
}
...
func createDebPackages() {
	createFpmPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/influxdb-srelay",
		binPath:                "/usr/sbin/influxdb-srelay",
		configDir:              "/etc/influxdb-srelay",
		configFilePath:         "/etc/influxdb-srelay/influxdb-srelay.conf",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/influxdb-srelay",
		initdScriptFilePath:    "/etc/init.d/influxdb-srelay",
		systemdServiceFilePath: "/usr/lib/systemd/system/influxdb-srelay.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/influxdb-srelay",
		defaultFileSrc: "packaging/deb/default/influxdb-srelay",
		systemdFileSrc: "packaging/deb/systemd/influxdb-srelay.service",
...
func createRpmPackages() {
	createFpmPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/influxdb-srelay",
		initdScriptFilePath:    "/etc/init.d/influxdb-srelay",
		systemdServiceFilePath: "/usr/lib/systemd/system/influxdb-srelay.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/influxdb-srelay",
		defaultFileSrc: "packaging/rpm/sysconfig/influxdb-srelay",
		systemdFileSrc: "packaging/rpm/systemd/influxdb-srelay.service",
...
func createMinTar() {
	packageRoot, _ := ioutil.TempDir("", "influxdb-srelay-linux-pack")
	// create directories
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/influxdb-srelay"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/influxdb-srelay/conf"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/influxdb-srelay/bin"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/influxdb-srelay/log"))
	//runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/influxdb-srelay/public"))
	runPrint("cp", "examples/sample.influxdb-srelay.conf", filepath.Join(packageRoot, "/opt/influxdb-srelay/conf"))
	runPrint("cp", "bin/influxdb-srelay", filepath.Join(packageRoot, "/opt/influxdb-srelay/bin"))
	runPrint("cp", "bin/influxdb-srelay.md5", filepath.Join(packageRoot, "/opt/influxdb-srelay/bin"))
	//runPrint("cp", "-a", filepath.Join(workingDir, "public")+"/.", filepath.Join(packageRoot, "/opt/influxdb-srelay/public"))
	tarname := fmt.Sprintf("dist/influxdb-srelay-%s-%s_%s_%s.tar.gz", version, getGitSha(), runtime.GOOS, runtime.GOARCH)
...
func createFpmPackage(options linuxPackageOptions) {
	...
	// copy sample ini file to /etc/influxdb-srelay
	runPrint("cp", "examples/sample.influxdb-srelay.conf", filepath.Join(packageRoot, options.configFilePath))

	args := []string{
		"-s", "dir",
		"--description", "Service to to route, filter and replicate for high availability InfluxDB data.",
		"-C", packageRoot,
		"--vendor", "influxdb-srelay",
		"--url", "http://github.org/toni-moreno/influxdb-srelay",
		"--license", "Apache 2.0",
...
func clean() {
	...
	//rmr("public")
	//rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/toni-moreno/influxdb-srelay", goos, goarch)))
}
